...
public class IoCRegRepository: ICommand
{
    ...
    public void Execute()
    {
        ...
        IoC.Resolve<ICommand>("IoC.Register", "GameItem.Add", (object[] args) => new RunnableCommand(
            () =>
            {
                IoC.Resolve<ICommand>("GameItem.AddByObjectId", ((IObjectInfo) args[0]).ObjectId, (IObjectInfo) args[0]).Execute();
                IoC.Resolve<ICommand>("GameItem.AddByPlayerId", ((IObjectInfo) args[0]).PlayerId, (IObjectInfo) args[0]).Execute();
            })).Execute();
        ...
        IoC.Resolve<ICommand>("IoC.Register", "GameItem.Remove", (object[] args) => new RunnableCommand(
            ...
            () =>
            {
                ...
                if (_objectsByPlayerIds.ContainsKey(((IObjectInfo) args[0]).PlayerId) && _objectsByPlayerIds[args[0]].Contains(args[0]))
